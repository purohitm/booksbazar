...
router.get('/:id', async (req, res) => {
    try {
        ...
        const book = await Book.findByPk(bookId, {
            attributes: {
                exclude: ['userId'] // Exclude the userId field
            }
        });
        ...
        // If the book doesn't have a price, set a default price
        if (!book.price) {
            book.price = 0;
        }
...
router.post('/:bookId/add-to-cart', auth, async (req, res) => {
    try {
        ...
        if (!book) {
            ...
            try {
                // Try to get the book from Google Books API
                const response = await axios.get('https://www.googleapis.com/books/v1/volumes', {
                    params: {
                        q: `isbn:${bookId}` || `isbn:${bookId.replace('-', '')}`
                    }
                });
...
const formatBook = (bookData) => {
    ...
    return {
        id: bookData.id || volumeInfo.id || Date.now().toString(),
        ...
        price: 19.99, // Default price for new books
