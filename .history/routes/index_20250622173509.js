...
// Add book to cart (requires login)
router.post('/books/:id/add-to-cart', auth.requireAuth, async (req, res) => {
    try {
        const { id } = req.params;
        ...
        req.flash('error', 'Error adding book to cart');
        res.redirect(`/books/${id}`);
    }
});

// Sell a book (requires seller role)
router.get('/books/sell', auth.requireSeller, (req, res) => {
