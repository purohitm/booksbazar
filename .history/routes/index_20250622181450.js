...
router.get('/books/search', async (req, res) => {
    try {
        ...
        // Get total items from the API response
        const totalItems = response?.data?.totalItems || 0;
        
        // If totalItems is not provided by API, estimate based on results
        const estimatedTotalItems = totalItems || (response?.data?.items?.length || 0) * 10;
        
        // Format the books
        const books = (response?.data?.items || []).map(book => formatBook(book));

        // Calculate pagination
        const totalPages = Math.ceil(estimatedTotalItems / itemsPerPage);
