...
router.get('/:id', async (req, res) => {
    try {
        ...
        // TEMPORARY FIX: For testing, assign current user as owner if no owner exists
        // This should be removed in production and proper ownership should be set during book creation
        if (!book.userId && req.user) {
            console.log(`⚠️  Book ${book.id} has no owner, temporarily assigning to user ${req.user.id} for testing`);
            // Update the book in database with current user as owner
            await Book.update(
                { userId: req.user.id },
                { where: { id: book.id } }
            );
            book.userId = req.user.id; // Update local object too
        }
        ...
        try {
            ...
            if (!challenge) {
                ...
                challenge = await Challenge.findOne({
                    ...
                    attributes: ['id', 'question', 'difficulty', 'createdBy']
                });
            }
            ...
            if (!challenge) {
                ...
                challenge = await Challenge.findOne({
                    ...
                    attributes: ['id', 'question', 'difficulty', 'createdBy', 'isActive']
                });
            ...
            if (challenge) {
                console.log('🔍 Challenge details:', {
                    ...
                    question: challenge.question.substring(0, 50) + '...'
                });
            }
            ...
            if (challenge) {
                ...
                if (req.user) {
                    ...
                    console.log('🔍 User is creator?', challenge.createdBy === req.user.id);
